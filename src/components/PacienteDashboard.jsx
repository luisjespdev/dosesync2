...
// IMPORTACIÓN DE CAPACITOR PARA NOTIFICACIONES GLOBALES
...
export default function PacienteDashboard({ userData, activeTab, dispararAlarma }) {
  ...
  // FUNCIÓN PARA PROGRAMAR NOTIFICACIÓN NATIVA (TELÉFONO CERRADO)
  const programarNotificacionNativa = async (m) => {
    try {
      const now = new Date();
      const [hh, mm] = m.hora.split(':').map(Number);
      let scheduleDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), hh, mm, 0);
      
      if (scheduleDate <= now) {
        scheduleDate.setDate(scheduleDate.getDate() + 1);
      }

      // Convertir el ID de Firebase (string) a un número entero para Capacitor
      const numericId = Math.abs(m.id.split('').reduce((a, b) => {
      ...
      await LocalNotifications.schedule({
        notifications: [
          {
            title: "Recordatorio de DoseSync",
            body: `Es hora de tu dosis: ${m.nombre} (${m.dosis})`,
            id: numericId,
            schedule: { at: scheduleDate, allowWhileIdle: true }, // Permite sonar en modo reposo
            sound: 'res://raw/notification_sound.mp3', 
            vibrate: true,
            importance: 5, // Prioridad máxima
            extra: { medName: m.nombre }
          }
        ]
      });
    } catch (e) {
      console.error("Error programando notificación nativa:", e);
    }
  };
  ...
  useEffect(() => {
    ...
    const unsubMed = onValue(medsRef, async (snapshot) => {
      const data = snapshot.val();
      ...
      listaMeds.forEach(m => {
        ...
        // 1. Alarma interna para App abierta
        timeoutsRef.current[m.id] = setTimeout(() => {
          dispararAlarma(m.nombre, m.hora, m.dosis);
        }, delay);

        // 2. Notificación global para App cerrada
        programarNotificacionNativa(m);
      });
    });
  ...
  return (
    <div className="dashboard-container">
      {/* El contenido JSX se mantiene íntegro según tu última versión */}
      {activeTab === 'home' && (
        <div className="animate-in">
          ...
          <div className={`curiosidad-card medical-note ${notaDelMedico?.leida ? '' : 'unread'}`} 
            ...
            {notaDelMedico?.mensaje && notaDelMedico.mensaje !== "Sin instrucciones nuevas." && !notaDelMedico.leida && (
               <button onClick={confirmarLecturaNota} className="btn-confirm-read"
      ...
      {activeTab === 'recordatorios' && (
        <div className="animate-in">
          ...
          <FormularioMedicamento medicamentoAEditar={editando} alTerminar={() => setEditando(null)} />
          <h3 style={{ marginTop: '1.5rem' }}>Mis Medicamentos</h3>
          <ul className="lista-medicamentos">
            {medicamentos.length === 0 ? <p className="historial-empty">No tienes medicamentos guardados.</p> : (
              medicamentos.map(m => (
                <li key={m.id} className="medicamento-item card-shadow">
                  ...
                  <div className="medicamento-actions">
                    <button className="btn-icon" onClick={() => setEditando(m)}><Pencil size={18} color="#3498db" /></button>
                    <button className="btn-icon" onClick={() => eliminarMedicamento(m.id)}><Trash2 size={18} color="#e74c3c" /></button>
                  </div>
                </li>
              ))
            )}
      ...
      {activeTab === 'historial' && (
        <div className="animate-in">
          ...
          <ul className="lista-historial">
            {historial.length === 0 ? <p className="historial-empty">Aún no hay registros en tu historial.</p> : (
              historial.map(h => (
                <li key={h.id} className="historial-item card-shadow" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      {h.motivoOmision && <small style={{ display: 'block', color: '#d35400', backgroundColor: '#fdf2e9', padding: '2px 5px', borderRadius: '4px', marginTop: '2px', fontStyle: 'italic' }}>Nota: {h.motivoOmision}</small>}
                      <small style={{ color: '#888' }}><Clock size={12} style={{ verticalAlign: 'middle', marginRight: '3px' }} />{h.hora} - {new Date(h.fecha).toLocaleDateString()}</small>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontWeight: 'bold', color: h.estado === 'tomado' ? '#2ecc71' : '#e74c3c', fontSize: '0.9rem' }}>
                    {h.estado === 'tomado' ? <><CheckCircle2 size={18} /> Tomado</> : <><XCircle size={18} /> Omitido</>}
                  </div>
                </li>
              ))
            )}
